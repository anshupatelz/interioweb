...
const mainMenu: NavItem[] = [
    { name: "Home", link: "https://aimpur.com/" },
    {
        name: "All Tools",
        link: "https://aimpur.com/",
        children: [
            { name: "Percentage to CGPA", link: "/percentage-to-cgpa-calculator/" },
            { name: "CGPA to Percentage", link: "/cgpa-to-percentage-calculator/" },
            { name: "Attendance Calculator", link: "/attendance-percentage-calculator/" },
            { name: "Age Calculator", link: "/age-calculator/" },
            { name: "Marks to Percentage", link: "/marks-to-percentage-calculator/" },
        ],
    },
    { name: "Blog", link: "https://aimpur.com/blog/" },
    { name: "About", link: "https://aimpur.com/about/" },
]
...
export default function Navbar() {
    ...
    return (
        ...
        <nav className="fixed top-4 left-1/2 transform -translate-x-1/2 z-50 w-full px-4 container">
            <div className="bg-white backdrop-blur-md border border-border/50 rounded-3xl shadow-lg">
                <div className="flex items-center justify-between h-12 px-4">
                    <Link href="/" className="text-xl font-bold text-primary">
                        <Image src="/aimpur-tools-logo.png" alt="Aimpur Tools" width={160} height={32} />
                    </Link>
                                                ...
                                                <div className="grid grid-cols-1 gap-1 p-4 text-popover-foreground rounded-lg shadow-lg border min-w-[16rem]">
                                                    {item.children.map((child) => (
                                                        <NavigationMenuLink key={child.link} asChild>
                                                            <Link
                                                                href={child.link}
                                                                className="block rounded-md px-3 py-2 text-sm font-medium text-muted-foreground hover:bg-muted hover:text-primary transition-colors"
                                                            >
                                                                {child.name}
                                                            </Link>
                                                        </NavigationMenuLink>
